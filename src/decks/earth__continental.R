! Earth Continental deck, Hannah Woodward 2024
...
!    - ocean: Earth-like ICs with bathtub
...
GIC=GIC.E046D3M20A.1DEC1955.ext_1.nc   ! initial conditions (ground) aquaplanet (ssi=0)
...
!! ----- CUSTOM TOPOGRAPHY
TOPO=Z72X46N_gas.1_nocasp_btub005.nc
OSTRAITS=OSTRAITS_72x46btub0.nml
RVR=RD_modelE_M_btub004D.nc
NAMERVR=RD_modelE_M.names_btub0.txt
VEG=V72X46.1.cor2_no_cropsYANG.ext.nc  ! albedo = 0.3
SOIL=S4X50093YANG.ext.nc               ! 50/50 sand + clay soil type
TOP_INDEX=top_index_72x46_a.ij.ext.nc  ! only used if #define DO_TOPMODEL_RUNOFF

! Only used for carbon cycle runs, i.e. if you have a non-zero vegetation fraction and want to know the CO2 flux from vegetated soil.
...
!! ----- UPDATE THE LABEL
Label and Namelist:
earth__continental (1880 atm.,the current modelE version)
...
spectral_file_lw='sp_lw_dsa_ar/sp_lw_15_dsa_ar'
spectral_file_sw='sp_sw_dsa_ar/sp_sw_43_dsa_ar'
...
eccentricity=0.
obliquity=0.
siderealorbitalPeriod=31536000.0d0 ! unit seconds; 365 days
siderealRotationPeriod=86400.0d0   ! unit seconds; 1 day
...
KSIALB=0            ! 6-band albedo (Hansen) (=-1 no land ice fixup, =1 A.Lacis orig. 6-band alb)
...
KRSF=600            ! save one checkpoint restart file at the beginning of every X month(s)
...
Ndisk=480
nrad=2              ! radiation performed every nrad'th physics time step
ndigits_year=5
!nmonav=1
!keep_params=1
